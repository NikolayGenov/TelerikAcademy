...
namespace AcademyRPG
{
    ...
    public class ExtendedEngine : Engine
    {
        ...
        public override void ExecuteControllableCommand(string[] commandWords)
        {
            ...
            if (current != null && commandWords[1] == "attack")
            {
                HandleAttackCommand(current);
            }
            else
    ...
    {
        public Guard(string name, Point position, int owner) : base(name, position, owner)
        {
            this.HitPoints = 100;
        }
    ...
    {
        public Lumberjack(string name, Point position, int owner) : base(name, position, owner)
        {
            this.HitPoints = 50;
        }
    ...
    public class Ninja : Character, IGatherer, IFighter
    {
        ...
        public int GetTargetIndex(List<WorldObject> availableTargets)
        {
            for (int i = 0; i < availableTargets.Count; i++)
            {
                WorldObject playerWanted;
                var highestPlayers = from player in availableTargets
                                     orderby player.HitPoints ascending
                                     select player;
                if (highestPlayers.First() == this)
                {
                    var playersArr = highestPlayers.ToArray();
                    playerWanted = playersArr[1];
                }
                else
                {
                    playerWanted = highestPlayers.First();
                }
                if (availableTargets[i].Owner != this.Owner && availableTargets[i].Owner != 0 &&
                    playerWanted == availableTargets[i])
                {
                    return i;
                }
            }
    ...
    public class Tree : StaticObject, IResource
    {
        ...
        public Tree(int size, Point position) : base(position)
        {
            this.Size = size;
            this.HitPoints = size;
        }
